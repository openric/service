    ...
    <section class="mb-4" x-data="{ open: false }">
        <h2 class="h5">
            ...
            @if (count($inheritedAttributes) > 0)
                <button type="button" class="btn btn-sm btn-link" @click="open = !open" x-text="open ? 'Hide' : 'Show'" :aria-expanded="open" aria-controls="expandable"></button>
            @endif
        </h2>
        @if (count($inheritedAttributes) > 0)
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="ric-tag ric-tag-inherited">
                                        from <a href="{{ route('reference.ric-cm.entities.show', ['version' => $version, 'id' => $a['inheritedFrom']['id']]) }}" class="text-reset">{{ $a['inheritedFrom']['name'] }}</a>
                                    </span>
                                </td>
                            </tr>
                        @endforeach
                    ...
                    <tbody>
                        @foreach ($inheritedRelations as $r)
                            <tr>
                                ...
                                <td>
                                    <span class="ric-tag ric-tag-inherited">
                                        from <a href="{{ route('reference.ric-cm.entities.show', ['version' => $version, 'id' => $r['inheritedFrom']['id']]) }}" class="text-reset">{{ $r['inheritedFrom']['name'] }}</a>
                                    </span>
                                </td>
                            </tr>
                        @endforeach
