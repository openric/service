    ...
    public static function build(string $baseUrl): array
    {
        ...
        return [
            ...
            'tags' => [
                ...
                ['name' => 'Activities','description' => 'rico:Activity — every event class is rico:Activity (RiC-O 1.1 has no concrete subclasses); kind via rico:hasActivityType IRI from <https://openric.org/vocab/activity-type/>.'],
                ['name' => 'Instantiations', 'description' => 'rico:Instantiation — carriers of bitstreams (files, images).'],
                ['name' => 'Repositories', 'description' => 'ISDIAH repository surface — canonical: rico:CorporateBody with rico:hasOrHadHolder relations (per spec v0.37 §6.3); /repositories is an API convenience route.'],
                ['name' => 'Functions',    'description' => 'ISDF function surface — interim canonical: openricx:Function (per spec v0.37 §6.4); RiC-O 1.1 has no rico:Function class.'],
                ['name' => 'Relations', 'description' => 'rico:Relation — typed links between entities.'],
                ['name' => 'Graph',     'description' => 'Subgraph walks via /graph. SPARQL is non-normative for OpenRiC core; servers MAY expose /sparql under the optional sparql-access Draft profile.'],
