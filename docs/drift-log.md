...
- **2026-04-21** | `divergence` | `packages/ahg-ric/src/Services/RicSerializationService.php:1101` | heratio@unchanged | openric@v0.8.12 (pending) | **Bug fix to align with spec v0.3 `rico:ContactPoint` shape.** Contact object `@type` changed from `rico:Contact` (not a valid RiC-O class) to `rico:ContactPoint` (the RiC-native class committed to in `spec/mapping.md` §5.2.2 and now normative in `spec/profiles/core-discovery.md` §3.4.1). Heratio's identical line carries the same bug and is NOT fixed here. Direction for any future reconciliation: `openric→heratio`.
